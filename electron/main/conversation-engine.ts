...
export class ConversationEngine {
  ...
  private async runAcpTurn(
    ...
    if (agent.kind === 'codex-cli') {
      await this.runCodexCliTurn(conversationId, userMessageId, turnId, turnPrompt, agent);
      return;
    }
  ...
  private async runCodexCliTurn(
    ...
    try {
      ...
      const body = await runCodexCli(agent.command, agent.args, agent.cwd || process.cwd(), prompt);
...
function runCodexCli(
  ...
  return new Promise((resolve, reject) => {
    void runCodexCliInner(command, args, cwd, prompt).then(resolve, reject);
  });
}
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      child.stdout.on('data', () => {
        // Codex prints progress to stdout; drain it so the child process cannot block on a full pipe.
      });
